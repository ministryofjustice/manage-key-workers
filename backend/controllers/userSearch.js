...
const adminSearch = function (eliteApi, res, agencyId, nameFilter, roleFilter) {
  return eliteApi.userSearchAdmin(
    res.locals,
    {
      agencyId,
      nameFilter,
...
const userSearchFactory = (eliteApi) => {
  const userSearch = asyncMiddleware(async (req, res) => {
    ...
    const response = hasAdminRole === 'true' ? await adminSearch(eliteApi, res, agencyId, nameFilter, roleFilter) : await search(eliteApi, res, agencyId, nameFilter, roleFilter);
