...
const keyworkerStatsFactory = (keyworkerApi) => {
  const getStatsForStaffRoute = asyncMiddleware(async (req, res) => {
    const { staffId, agencyId, fromDate, toDate, period } = req.query;
    const stats = await getStatsForStaff({ locals: res.locals, staffId, agencyId, fromDate, toDate, period });
    log.debug({ data: stats }, 'Response from keyworker stats request');
    res.json(stats);
  });

  const getStatsForStaff = async ({ locals, staffId, agencyId, fromDate, toDate, period }) => {
    const getStats = [
...
const complianceRate = (currentStats, pastStats, period) => ({
  ...
  change: {
    value: pastStats && currentStats.complianceRate - pastStats.complianceRate,
    period
  }
});
