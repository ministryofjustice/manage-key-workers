const userMeFactory = (hmppsManageUsersApi, elite2Api, keyworkerApi) => {
  const userMeService = async (req, res) => {
    const context = res.locals
    const user = await hmppsManageUsersApi.currentUser(context)
    ...
    const roles = await hmppsManageUsersApi.currentRoles(context)
