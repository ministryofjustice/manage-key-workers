        ...
        cy.get('[data-test="results-table"]')
          ...
          .then(($tableRows) => {
            ...
            cy.get(offenders[0].changeKeyworker)
              .find('[data-test="allocate-keyworker-select"]')
              .then(($select) => {
                cy.get($select).find('option:selected').contains('6 - Ball, Bob')
                cy.get($select)
                  .find('option')
                  .then(($options) => {
            ...
            cy.get(offenders[1].changeKeyworker)
              .find('[data-test="allocate-keyworker-select"]')
              .then(($select) => {
                cy.get($select).find('option:selected').contains('1 - Smith, Andy')
                cy.get($select)
                  .find('option')
                  .then(($options) => {
                    cy.get($options).its('length').should('eq', 4)
                    expect($options.get(0)).to.contain('Select key worker')
                    expect($options.get(1)).to.contain('1 - Smith, Andy')
                    expect($options.get(2)).to.contain('6 - Ball, Bob')
                    expect($options.get(3)).to.contain('9 - Doe, Julian')
                  })
              })
            cy.get(offenders[1].viewHistory)
              .find('a')
              ...
              .then(($select) => {
                cy.get($select)
                  ...
                  .then(($options) => {
                    ...
                    expect($options.get(0)).to.contain('Select key worker')
                    expect($options.get(1)).to.contain('9 - Doe, Julian')
                  })
              })
            cy.get(offenders[0].viewHistory).find('a').should('not.exist')
