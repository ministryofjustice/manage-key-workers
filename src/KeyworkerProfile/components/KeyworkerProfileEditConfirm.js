  ...
  render () {
    ...
    if (status !== 'INACTIVE') {
      innerContents = (<div><div className="pure-u-md-6-12">
        ...
        <div className="multiple-choice">
          <input type="radio" name="allocationOption" value="REMOVE_ALLOCATIONS_NO_AUTO" onClick={handleOptionChange}/>
      ...
      {(status === 'UNAVAILABLE_ANNUAL_LEAVE') && (
        <div className="pure-u-md-8-12 padding-top bold">
          ...
          <div className="pure-u-md-5-12" id="active-date" >
            <DatePickerInput className="annualLeaveDate" handleDateChange={handleDateChange} additionalClassName="dateInput" inputId="search-date" />
          </div>
        </div>)}
          ...
          <div className="pure-g">
            <div className="pure-u-md-4-12">
              ...
              <div id="keyworker-status" name="keyworker-status"
                className={`${statusStyle}Status margin-top`}>{getStatusDescription(status)}</div>
            </div>
          </div>
              ...
              <div className="pure-u-md-10-12">
                <div className="buttonGroup">
                  <button className="button button-save"
                    onClick={() => handleSaveChanges(history)}>Save changes
                  </button>
                </div>
                <div className="buttonGroup">
                  <button className="button greyButton button-cancel"
                    onClick={() => handleCancel(history)}>Cancel</button>
                </div>
              </div>
            </div>
          </div>
