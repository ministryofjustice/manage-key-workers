  ...
  render () {
    ...
    const buttons = (<div>
      <button className="button button-save"
        onClick={() => postManualOverride(history)}>Confirm allocation
      </button>
      <button className="button greyButton button-cancel margin-left"
        onClick={() => onFinishAllocation(history)}>Cancel allocation
      </button>
    </div>);
    ...
    return (
      <div>
        <div className="pure-g">
          ...
          {displayDateFilter && <div className="pure-u-md-5-12"><DateFilter {...this.props} /></div>}
        </div>
