...
describe('AllocateParent component', () => {
  it('should get page 1 correctly', async () => {
    ...
    await shallow(<AllocateParent jwt={"dummy-jwt"} agencyId={'LEI'} onFinishAllocation={jest.fn()} setErrorDispatch={jest.fn()} unallocatedListDispatch={jest.fn()}
  ...
  it('should go to page 2 correctly', async () => {
    ...
    const component = await shallow(<AllocateParent jwt={"dummy-jwt"} agencyId={'LEI'} onFinishAllocation={jest.fn()} setErrorDispatch={jest.fn()} unallocatedListDispatch={jest.fn()}
      manualOverrideDispatch={jest.fn()} manualOverrideDateFilterDispatch={jest.fn()} setCurrentPageDispatch={jest.fn()} allocatedDetailsDispatch={jest.fn()}/>);
  ...
  it('should render a middle tier error on page 1 correctly', (done) => {
    ...
    const component = shallow(<AllocateParent jwt={"dummy-jwt"} agencyId={'LEI'} onFinishAllocation={jest.fn()} setErrorDispatch={jest.fn()} unallocatedListDispatch={jest.fn()}
